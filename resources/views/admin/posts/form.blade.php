<x-admin.layout title="{{ $action === 'edit' ? 'Modifier : ' . $post->title : 'Nouvel article' }}">
    <div class="max-w-2xl">
        <div class="mb-6"><a href="{{ route('admin.posts.index') }}"
                class="text-[var(--muted)] hover:text-[var(--text)] text-sm">←
                Retour</a></div>
        <div class="glass rounded-xl p-6">
            <form method="POST"
                ...
                <div class="grid sm:grid-cols-3 gap-5">
                    <div>
                        <label class="block text-sm font-medium text-[var(--muted)] mb-1.5">Catégorie <span
                                class="text-[var(--error)]">*</span></label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-[var(--muted)] mb-1.5">Date publication</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-[var(--muted)] mb-1.5">Extrait <span
                            class="text-[var(--error)]">*</span></label>
                ...
                <div>
                    ...
                    <p class="text-xs text-[var(--muted)] opacity-60 mt-1">Supporte le Markdown basique</p>
                </div>
                <div class="flex items-center gap-3">
                    <input type="checkbox" name="active" id="active" value="1" class="w-4 h-4 accent-[var(--primary)]"
                        {{ old('active', $post->active ?? true) ? 'checked' : '' }}>
                    <label for="active" class="text-sm text-[var(--muted)]">Article actif / visible</label>
                </div>
